...
fn calcscore(f: &String) -> Result<(), String>{
    ...
    for dict in net_scores {
        handle.write_fmt(format_args!("{{\"URL\":{:?}, ", dict.get("URL").unwrap())).unwrap();
        handle.write_fmt(format_args!("\"NET_SCORE\":{:.2}, ", dict.get("NET_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
        handle.write_fmt(format_args!("\"RAMP_UP_SCORE\":{:.2}, ", dict.get("RAMP_UP_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
        handle.write_fmt(format_args!("\"CORRECTNESS_SCORE\":{:.2}, ", dict.get("CORRECTNESS_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
        handle.write_fmt(format_args!("\"BUS_FACTOR_SCORE\":{:.2}, ", dict.get("BUS_FACTOR_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
        handle.write_fmt(format_args!("\"RESPONSIVE_MAINTAINER_SCORE\":{:.2}, ", dict.get("RESPONSIVE_MAINTAINER_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
        handle.write_fmt(format_args!("\"LICENSE_SCORE\":{}}}\n", dict.get("LICENSE_SCORE").unwrap().parse::<f64>().unwrap())).unwrap();
    }
    Ok(())
}